...
class CephRoles(object):
    ...
    def _client_roles(self):
        ...
        """
        roles = [ 'client-cephfs', 'client-rbd', 'client-radosgw', 'client-iscsi', 'client-nfs'  ]
        self.available_roles.extend(roles)
    ...
    def cephfs_members(self):
        """
        Create a file for cephfs hosts.
        """
        minion_dir = "{}/role-cephfs-client/stack/default/{}/minions".format(self.root_dir, self.cluster)
        if not os.path.isdir(minion_dir):
    ...
    def rbd_members(self):
        """
        Create a file for rbd client hosts.
        """
        minion_dir = "{}/role-rbd-client/stack/default/{}/minions".format(self.root_dir, self.cluster)
        if not os.path.isdir(minion_dir):
            create_dirs(minion_dir, self.root_dir)
        for server in self.servers:
            filename = minion_dir + "/" +  server + ".yml"
